...
class PairDistanceCalculator(Calculator):
    ...
    def _calculate_distances(self):
        ...
        Let matix `distances` be a matrix of all possible L1 (Manhattan) distances between
    ...
    def _get_Q_vals(self, start: int, end: int) -> NDArray:
        ...
        `A` are given by matrix
              ...
              | A(start, start + 1, start + 2)    A(start, start + 1, start + 3)   …  A(start, start + 1, end + 1) |
              |                0                  A(start, start + 2, start + 3)   …  A(start, start + 2, end + 1) |
        A =   |                ⋮                                  ⋮                 ⋱                 ⋮            | .
              |                0                                  0                …  A(start, end, end + 1)       |
        ...
        V = self.V[start : end] - self.distances[0 : start, start + 1 : end + 1].sum(axis=0)
    ...
    def get_candidate_change_point(self, interval: slice) -> CandidateChangePoint:
        ...
        Function `QQ` is defined as the following:
            ...
            be a split of `Z`, i.e.,
                            X = [Z_{start}, Z_{start + 1}, ..., Z_a] = [X_1, X_2, ..., X_n],
                            Y = [Z_{a + 1}, Z_{a + 2}, ..., Z_b] = [Y_1, Y_2, ..., Y_m],
            then
                            QQ(X, Y) = 2 / (m + n) * Σi=1,n Σj=1,m |X_i - Y_j|
                                      - 2 * m / (m + n) / (n - 1) * Σ1<=i<k<=n |X_i - X_k|
                                      - 2 * n / (m + n) / (m - 1) * Σ1<=j<k<=m |Y_j - Y_k|.

            Note: QQ(X, Y) = QQ(Z[start : a + 1], Z[a + 1 : b + 1]) = Q(a + 1, b + 1).
        ...
        end = len(self.series) - 1 if interval.stop is None else interval.stop - 1
